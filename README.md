...
`sensor`/`text_sensor` platforms that map values by OBIS code.
...
- **amis_meter_id** (*Optional*): The hub to read from. Only needed with multiple
  hubs.
- **obis_code** (**Required**, string): Which value to report. Flexible
  notations are accepted (`1.8.0`, `1-0:1.8.0`, `1.0.1.8.0.255`); a 3-part
  code is expanded to `1.0.C.D.E.255`.
...
| `1.0.16.7.0.255` | 16.7.0 | Net active power (P+ − P-), computed | W |
